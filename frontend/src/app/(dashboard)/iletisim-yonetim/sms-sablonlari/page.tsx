...
import { Plus, Pencil, Trash2, X, MessageSquare, AlertCircle, Search, ChevronLeft, ChevronRight } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { Label } from '@/components/ui/label'
import { smsSablonlariApi } from '@/lib/api/iletisimYonetim'
import type { SmsSablonu, CreateSmsSablonuDto, UpdateSmsSablonuDto } from '@/types/iletisimYonetim'
import { showSuccess, showApiError } from '@/lib/toast'

export default function SmsSablonlariPage() {
  const [items, setItems] = useState<SmsSablonu[]>([])
  ...
  const [selected, setSelected] = useState<SmsSablonu | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await smsSablonlariApi.getAll(page, pageSize, search || undefined)
  ...
  const openCreate = () => {
    setFormAd(''); setFormIcerik(''); setFormKategori(''); setFormIsActive(true)
    setFormErrors({}); setSelected(null); setPanelMode('create'); setPanelOpen(true)
  }

  const openEdit = (item: SmsSablonu) => {
    setFormAd(item.ad); setFormIcerik(item.icerik); setFormKategori(item.kategori ?? ''); setFormIsActive(item.isActive)
  ...
  const validate = () => {
    ...
    if (formIcerik.length > 500) errors.icerik = 'En fazla 500 karakter.'
  ...
  const handleSave = async () => {
    ...
    try {
      if (panelMode === 'create') {
        const dto: CreateSmsSablonuDto = { ad: formAd.trim(), icerik: formIcerik.trim(), kategori: formKategori.trim() || undefined }
        await smsSablonlariApi.create(dto)
        showSuccess('SMS şablonu oluşturuldu.')
      } else if (selected) {
        const dto: UpdateSmsSablonuDto = { ad: formAd.trim(), icerik: formIcerik.trim(), kategori: formKategori.trim() || undefined, isActive: formIsActive }
        await smsSablonlariApi.update(selected.id, dto)
        showSuccess('SMS şablonu güncellendi.')
      }
      closePanel(); load()
    } catch { showApiError() }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await smsSablonlariApi.delete(id)
      showSuccess('SMS şablonu silindi.')
      ...
      <div className={`flex-1 min-w-0 transition-all duration-300 ${panelOpen ? 'pr-[460px]' : ''}`}>
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-bold tracking-tight">SMS Şablonları</h1>
              <p className="text-sm text-muted-foreground mt-1">Otomatik SMS bildirimlerinde kullanılacak şablonları yönetin.</p>
            </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-muted-foreground">Henüz SMS şablonu eklenmemiş.</p>
              ...
              <table className="w-full text-sm">
                <thead className="bg-muted/50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left font-medium text-muted-foreground hidden md:table-cell">İçerik</th>
                ...
                <tbody className="divide-y">
                  {items.map(item => (
                    <tr key={item.id} className={`hover:bg-muted/30 cursor-pointer transition-colors ${selected?.id === item.id ? 'bg-muted/50' : ''}`} onClick={() => openEdit(item)}>
                      ...
                      <td className="px-4 py-3 text-muted-foreground hidden md:table-cell truncate max-w-[240px]">{item.icerik}</td>
      ...
      {panelOpen && (
        <div className="fixed right-0 top-0 h-screen w-[460px] bg-background border-l shadow-2xl flex flex-col z-50">
          <div className="flex items-center justify-between px-6 py-4 border-b shrink-0">
            <h2 className="font-semibold text-base">{panelMode === 'create' ? 'Yeni SMS Şablonu' : 'Şablonu Düzenle'}</h2>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="SMS mesaj içeriği (max 500 karakter)"
                rows={5}
                maxLength={500}
